...
//            preloader(false)
    ...
    this.readFilecsv = function(file,separate,object) {
        main = main();
        separate = separate||'|';
        let db = "csv";
        firstTime= true;
...
//        preloader(true, 0,"Load data....");
        ...
        function loadcsv(data) {
            ...
            menuSetting.newdatatoFormat_noSuggestion(data, separate);
            if (object && object.customTime){
...
//            menuSetting.inithostResults();
            menuSetting.formatService(true);
...
//                    preloader(false);
        ...
        setTimeout(() => {
            if (file) {
                ...
                dataInformation.size = object?object.size: 0;
                if(object)loadcsv(object.values)
            }
        }, 0);
    }
//    Loaddata.reset();
...
export var loadDataService = new LoadDataService();
loadDataService.readFilecsv()
